  ...
  return (
    <Card>
      ...
      <div className="divide-y divide-[#232323]">
        {bas.length === 0 && (
          <div className="px-6 py-8 text-center text-[#f0ede8] text-sm">No ambassador data</div>
        )}
        {bas.map((ba, i) => (
          <Link key={ba.id} href={`/ambassadors/${ba.id}`}
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs text-[#f0ede8]">{ba.ba_tier ?? 'Standard'}</p>
            </div>
            <div className="text-right">
              <p className="text-sm font-semibold text-[#f0ede8] font-mono">{formatCurrency(ba.network_revenue)}</p>
              <p className="text-xs text-[#f0ede8]">network revenue</p>
            </div>
          </Link>
        ))}
      </div>
          ...
          <thead>
            <tr className="border-b border-[#232323]">
              {['Order #', 'Customer', 'Date', 'Total', 'Status'].map(h => (
                <th key={h} className="px-6 py-3 text-left text-xs font-medium text-[#f0ede8] uppercase tracking-widest">{h}</th>
              ))}
            </tr>
          </thead>
          <tbody className="divide-y divide-[#232323]">
            {orders.length === 0 && (
              <tr><td colSpan={5} className="px-6 py-8 text-center text-[#f0ede8]">No orders found</td></tr>
            )}
            {orders.map(o => (
              <tr key={o.id} className="hover:bg-[#1e1e1e] transition-colors">
                <td className="px-6 py-3 font-mono text-xs text-[#f0ede8]">{o.order_number}</td>
                <td className="px-6 py-3 text-[#f0ede8]">{o.customer_name}</td>
                <td className="px-6 py-3 text-[#f0ede8] font-mono text-xs">{formatDate(o.created_at)}</td>
...
async function SyncHealthSection() {
  ...
  return (
    <Card>
      ...
      <div className="divide-y divide-[#232323]">
        {logs.length === 0 && (
          <div className="px-6 py-8 text-center text-[#f0ede8] text-sm">No sync data</div>
        )}
        {logs.map(log => (
          <div key={log.id} className="flex items-center justify-between px-6 py-3">
            <div>
              <p className="text-sm text-[#f0ede8]">{log.source}</p>
              <p className="text-xs text-[#f0ede8]">{log.sync_type} · {timeAgo(log.completed_at ?? log.started_at)}</p>
            </div>
            <div className="flex items-center gap-3">
              {log.records_processed != null && (
                <span className="text-xs text-[#f0ede8] font-mono">{formatNumber(log.records_processed)} records</span>
              )}
  ...
  return (
    <div className="p-8 space-y-6 max-w-[1400px]">
      <div>
        ...
        <p className="text-[#f0ede8] text-sm">Cast Iron Nutrition · Internal Analytics</p>
      </div>
