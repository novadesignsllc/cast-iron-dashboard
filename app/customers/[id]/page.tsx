...
export default async function CustomerDetailPage({ params }: Props) {
  ...
  return (
    <div className="p-8 space-y-6 max-w-[1400px]">
      <Link href="/customers" className="inline-flex items-center gap-2 text-sm text-[#888580] hover:text-[#4ade9a] transition-colors">
        <ArrowLeft size={16} />
        Back to Customers
      </Link>
        ...
        <CardContent>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
            <div className="space-y-3">
              <div>
                <p className="text-xs text-[#888580] uppercase tracking-widest mb-1">Origin Source</p>
              ...
              <div>
                <p className="text-xs text-[#888580] uppercase tracking-widest mb-1">Referred By</p>
                {referredBy ? (
              ...
              <div>
                <p className="text-xs text-[#888580] uppercase tracking-widest mb-1">Origin Ambassador</p>
                {originBA ? (
            ...
            <div className="space-y-3">
              <div>
                <p className="text-xs text-[#888580] uppercase tracking-widest mb-1">Their Referral Code</p>
              ...
              <div>
                <p className="text-xs text-[#888580] uppercase tracking-widest mb-1">Code Uses</p>
                <p className="text-sm font-mono text-[#f0ede8]">{formatNumber(downstream.length)} people</p>
              </div>
              <div>
                <p className="text-xs text-[#888580] uppercase tracking-widest mb-1">Revenue from Referrals</p>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-[#232323]">
              {orders.length === 0 && (
                <tr><td colSpan={5} className="px-5 py-8 text-center text-[#888580]">No orders</td></tr>
              )}
              {orders.map(o => (
        ...
        <Card>
          ...
          <CardContent>
            <div className="grid grid-cols-3 gap-4">
              <div>
                <p className="text-xs text-[#888580] uppercase tracking-widest mb-1">Total Opens</p>
                <p className="text-2xl font-semibold font-mono text-[#f0ede8]">{emailEng.total_opens}</p>
              </div>
              <div>
                <p className="text-xs text-[#888580] uppercase tracking-widest mb-1">Total Clicks</p>
                <p className="text-2xl font-semibold font-mono text-[#f0ede8]">{emailEng.total_clicks}</p>
              </div>
              <div>
                <p className="text-xs text-[#888580] uppercase tracking-widest mb-1">Last Engaged</p>
