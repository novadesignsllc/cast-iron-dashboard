...
export default async function CustomerDetailPage({ params }: Props) {
  ...
  return (
    <div className="p-8 space-y-6 max-w-[1400px]">
      <Link href="/customers" className="inline-flex items-center gap-2 text-sm text-[#f0ede8] hover:text-[#4ade9a] transition-colors">
        <ArrowLeft size={16} />
        Back to Customers
      </Link>
          ...
          <div className="flex items-start gap-6">
            ...
            <div className="flex-1">
              <div className="flex items-center gap-3 flex-wrap mb-2">
                ...
                {customer.subscription_status && <Badge variant={customer.subscription_status}>{customer.subscription_status}</Badge>}
                {customer.klaviyo_churn_risk && <Badge variant={customer.klaviyo_churn_risk}>Churn: {customer.klaviyo_churn_risk}</Badge>}
              </div>
        ...
        <CardContent>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
            <div className="space-y-3">
              <div>
                <p className="text-xs text-[#f0ede8] uppercase tracking-widest mb-1">Origin Source</p>
              ...
              <div>
                <p className="text-xs text-[#f0ede8] uppercase tracking-widest mb-1">Referred By</p>
                {referredBy ? (
                ...
                ) : <p className="text-sm text-[#f0ede8]">—</p>}
              </div>
              <div>
                <p className="text-xs text-[#f0ede8] uppercase tracking-widest mb-1">Origin Ambassador</p>
                {originBA ? (
            ...
            <div className="space-y-3">
              <div>
                <p className="text-xs text-[#f0ede8] uppercase tracking-widest mb-1">Their Referral Code</p>
                {customer.ba_referral_code ? (
                  <code className="text-sm font-mono bg-[#111111] px-2 py-1 rounded border border-[#232323] text-[#4ade9a]">
                    {customer.ba_referral_code}
                  </code>
                ) : <p className="text-sm text-[#f0ede8]">No code assigned</p>}
              </div>
              <div>
                <p className="text-xs text-[#f0ede8] uppercase tracking-widest mb-1">Code Uses</p>
                <p className="text-sm font-mono text-[#f0ede8]">{formatNumber(downstream.length)} people</p>
              </div>
              <div>
                <p className="text-xs text-[#f0ede8] uppercase tracking-widest mb-1">Revenue from Referrals</p>
                <p className="text-sm font-semibold font-mono text-[#4ade9a]">{formatCurrency(downstreamRevenue)}</p>
              </div>
            </div>
          </div>
          <div className="mt-4 pt-4 border-t border-[#232323] flex gap-6">
            <div className="flex items-center gap-2 text-sm text-[#f0ede8]">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-[#232323]">
              {orders.length === 0 && (
                <tr><td colSpan={5} className="px-5 py-8 text-center text-[#f0ede8]">No orders</td></tr>
              )}
              {orders.map(o => (
                <tr key={o.id} className="hover:bg-[#1e1e1e] transition-colors">
                  ...
                  <td className="px-5 py-3 text-[#f0ede8] font-mono text-xs">{formatDate(o.created_at)}</td>
                  <td className="px-5 py-3 font-semibold font-mono text-[#f0ede8]">{formatCurrency(o.total)}</td>
                  <td className="px-5 py-3 text-xs text-[#f0ede8] font-mono">{o.discount_code ?? '—'}</td>
              ...
              <tbody className="divide-y divide-[#232323]">
                {refunds.map(r => (
                  <tr key={r.id} className="hover:bg-[#1e1e1e]">
                    <td className="px-5 py-3 text-[#f0ede8] font-mono text-xs">{formatDate(r.created_at)}</td>
                    <td className="px-5 py-3 font-semibold font-mono text-[#f05a5a]">{formatCurrency(r.amount)}</td>
                    <td className="px-5 py-3 text-xs text-[#f0ede8]">{r.reason ?? '—'}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <Card>
          ...
          <CardContent>
            <div className="grid grid-cols-3 gap-4">
              <div>
                <p className="text-xs text-[#f0ede8] uppercase tracking-widest mb-1">Total Opens</p>
                <p className="text-2xl font-semibold font-mono text-[#f0ede8]">{emailEng.total_opens}</p>
              </div>
              <div>
                <p className="text-xs text-[#f0ede8] uppercase tracking-widest mb-1">Total Clicks</p>
                <p className="text-2xl font-semibold font-mono text-[#f0ede8]">{emailEng.total_clicks}</p>
              </div>
              <div>
                <p className="text-xs text-[#f0ede8] uppercase tracking-widest mb-1">Last Engaged</p>
              ...
              <tbody className="divide-y divide-[#232323]">
                {downstream.map(c => (
                  <tr key={c.id} className="hover:bg-[#1e1e1e] transition-colors">
                    ...
                    <td className="px-5 py-3 text-[#f0ede8] font-mono text-xs">{formatDate(c.created_at)}</td>
                    ...
                    <td className="px-5 py-3">
                      {c.subscription_status ? <Badge variant={c.subscription_status}>{c.subscription_status}</Badge> : <span className="text-[#f0ede8]">—</span>}
                    </td>
                  </tr>
                ))}
              </tbody>
