  ...
  return (
    <div className="p-8 space-y-6 max-w-[1400px]">
      <div>
        ...
        <p className="text-[#f0ede8] text-sm">Data pipeline status and sync history</p>
      </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-[#232323]">
              {sourceLogs.length === 0 && (
                <tr><td colSpan={8} className="px-5 py-8 text-center text-[#f0ede8]">No sync data</td></tr>
              )}
              {sourceLogs.map(log => (
                <tr key={log.id}
                  ...
                  <td className="px-5 py-3 text-[#f0ede8] text-xs">{log.sync_type}</td>
                  <td className="px-5 py-3"><Badge variant={log.status}>{log.status}</Badge></td>
                  <td className="px-5 py-3 text-[#f0ede8] font-mono text-xs">{timeAgo(log.completed_at ?? log.started_at)}</td>
      ...
      <Card>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-[#232323]">
              {allLogs.length === 0 && (
                <tr><td colSpan={7} className="px-5 py-8 text-center text-[#f0ede8]">No sync history</td></tr>
              )}
              {allLogs.map(log => (
                <tr key={log.id}
                  className={`hover:bg-[#1e1e1e] transition-colors ${log.status === 'failed' ? 'bg-[rgba(240,90,90,0.04)]' : ''}`}>
                  <td className="px-5 py-3 text-[#f0ede8] text-xs whitespace-nowrap font-mono">
                  ...
                  <td className="px-5 py-3 text-xs text-[#f0ede8]">{log.sync_type}</td>
                  <td className="px-5 py-3"><Badge variant={log.status}>{log.status}</Badge></td>
                  <td className="px-5 py-3 tabular-nums font-mono text-[#f0ede8]">{log.records_processed != null ? formatNumber(log.records_processed) : '—'}</td>
